...
async function kick(i) {
  ...
  if (!target.kickable) return i.reply({ content: '⚠️ Ga bisa kick member ini (role Bot EXP mungkin di bawah dia, atau dia admin).', ephemeral: true });
...
async function ban(i) {
  ...
  const target = i.options.getMember('user');
  const reason = i.options.getString('reason') || 'Tanpa alasan';
  if (target && !target.bannable) return i.reply({ content: '⚠️ Ga bisa ban member ini (cek posisi role Bot EXP / dia admin).', ephemeral: true });
